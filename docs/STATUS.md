...
| Python API | Version 0.7 adds exact-byte audit trails/spans, an authenticated four-image firmware corpus, a retry-bounded simulator-only loader state machine, programmed-memory comparison, loader/controller fault injection, and structured interrupted workflow receipts; physical state-changing and loader paths remain blocked | 131 portable offline tests in the API-only export (one full-corpus test runs when preserved images are present), preserved serial 5215 replay fixtures, virtual endpoints, static analysis, and live read-only sessions |
