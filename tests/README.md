...
| Group | Files | Purpose |
| --- | --- | --- |
| Protocol and transport | `test_protocol.py`, `test_transport.py`, `test_runtime_safety.py`, `test_client.py`, `test_transactions.py` | Exact bytes, full-write enforcement, exclusive handles, durable recording, sleep/termination guards, parsing, timeouts, matching, and generic A/C/D behavior |
...
| Firmware laboratory | `test_firmware.py`, `test_firmware_catalog.py`, `test_loader.py`, `test_flashing.py`, `test_preservation.py`, `test_simulator.py` | Intel HEX, preserved corpus, loader rows/replies/retries/handoff, phase-matched delayed replies, zero-write rehearsal, exact rescue authentication and one-way delegation, recovery-state lifecycle, post-write diagnostic faults, guarded live-host gates, full-image faults, dump authentication, corruption, and isolated simulation |
